...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `embedder` | `Embedder` | *(required)* | Embedder instance for encoding text (e.g., `SentenceTransformerEmbedder`) |
...
freq_embedder = SentenceTransformerEmbedder("all-MiniLM-L6-v2")

freq_metrics = Toxicity.run(
    MyRetriever,
    embedder=freq_embedder,
    group_prototypes=group_prototypes,
    group_default_threshold=0.40,
    statistical_mode=FrequentistMode(),
...
bayes_embedder = SentenceTransformerEmbedder("all-MiniLM-L6-v2")

bayes_metrics = Toxicity.run(
    MyRetriever,
    embedder=bayes_embedder,
    group_prototypes=group_prototypes,
    group_default_threshold=0.40,
