...
<Note>
  BestOf works with all three modes, but the granularity changes. With `full_dataset` or `stream_sessions`, it compares full conversations as a block (one tournament per session). With `stream_batches`, it compares individual QA pairs (one tournament per question), producing one result per `qa_id` instead of per session.
</Note>
