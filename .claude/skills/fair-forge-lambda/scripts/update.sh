#!/usr/bin/env bash
#
# Update an existing Fair-Forge Lambda function with a new container image
#
# Usage: ./update.sh <module-name> [region]
#
# Example: ./update.sh bestof us-east-1
#
# Rebuilds the Docker image from fair-forge repo, pushes to ECR, and updates Lambda.
...
if [[ ! -f "$REPO_ROOT/pyproject.toml" ]]; then
    echo "Error: Cannot find fair-forge repo root at $REPO_ROOT"
    echo "Run this script from examples/<module>/aws-lambda directory"
    exit 1
fi

# --- Derived names ---
PROJECT_NAME="fair-forge-${MODULE_NAME}"
ECR_REPO="$PROJECT_NAME"
LAMBDA_NAME="$PROJECT_NAME"

echo "=== Updating Fair-Forge Lambda: $PROJECT_NAME ==="
