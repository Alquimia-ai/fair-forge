...
We propose to return a vector $\mathbf{m}$ of five normalized signals organized in two tiers. The mandatory core consists of two reference-free distributional metrics: the Consistency/Stability Rate (CSR), which measures how often the model produces semantically equivalent responses under repeated sampling, and normalized Semantic Entropy (SE), which quantifies uncertainty over the space of meanings. Three optional signals extend the core when dependencies are available: Reference Similarity Score (RSS), Instruction Compliance Rate (ICR), and Judge Quality (JQ). Scalar aggregation is left as an optional, explicitly calibrated step.
...
\[
\mathbf{m} = \left[\mathrm{CSR},\ 1 - \mathrm{SE}_n,\ \underbrace{\mathrm{RSS}^*,\ \mathrm{ICR}^*,\ \mathrm{JQ}^*}_{\text{optional (see Table~\ref{tab:components})}}\right]
\]
\end{definition}
...
The current implementation includes the mandatory core (CSR, SE) and the following Tier~2 components: RSS, ICR, and JQ.
