...
from fair_forge.schemas import Risk,GuardianBias,ProtectedAttribute
from abc import ABC, abstractmethod
from fair_forge import Guardian

    
class GraniteInfer(BaseModel):
    probability: float
    is_risk: bool

class GraniteProvider(ABC):
        def __init__(
            self,
            url,
            api_key,
        ...
        @abstractmethod
        def infer(self, prompt:partial) -> GraniteInfer:
...
class GraniteConfig(BaseModel):
    ...
    temperature: float = 0
    max_tokens: float = 5
    provider: Optional[Type[GraniteProvider]]
    

class IBMGuardian(Guardian):
    """
    Default implementation of the Guardian using the ibm granite model.
    """
    class HuggingFaceProvider(GraniteProvider):
        """
    ...
    class VllmProvider(GraniteProvider):
        """
        Uses OpenAI SLA.
        """
    ...
    def is_biased(self, question: str, answer: str, attribute: dict, context: Optional[str] = None) -> GuardianBias:
        pass

    def __init__(self, protected_attributes:list[ProtectedAttribute] | list[dict], config: GraniteConfig,**kwargs):
        super().__init__(protected_attributes=protected_attributes,**kwargs)
        self.url = config.url
        ...
        ## TODO: Parsing logic for protected attributes. (True risks)
    ...
    def _has_any_risk(self, question: str, answer: str, context: str) -> list[Risk]:
        """
